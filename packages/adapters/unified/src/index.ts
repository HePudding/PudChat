...
export function createChatModel(config: ProviderConfig): ChatModel {
  return {
    async *stream(
      ...
      const includeThinking = options.thinking === true;
      ...
      if (includeThinking) {
        body.reasoning = { tokens: options.maxTokens ?? 1024 };
      }
        ...
        if (!stream) {
          ...
          if (protocol === "openai") {
            ...
            if (reasoning && includeThinking)
              yield { type: "thinking", value: reasoning };
          } else {
            ...
            if (Array.isArray(content)) {
              for (const block of content) {
                ...
                if (block.type === "thinking" && block.text && includeThinking)
